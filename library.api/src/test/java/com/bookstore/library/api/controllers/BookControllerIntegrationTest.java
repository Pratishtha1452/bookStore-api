...
public class BookControllerIntegrationTest {
    ...
    public void testThatListBooksSuccessfullyReturnsListOfBooks() throws Exception {
        ...
        mockMvc.perform(
                ...
                MockMvcResultMatchers.jsonPath("$.content[0].isbn").value(booksEntity.getIsbn())
        ).andExpect(
                MockMvcResultMatchers.jsonPath("$.content[0].title").value(booksEntity.getTitle())
        );
    }
    ...
    @Test
    public void testThatDeleteBookDeletesExistingBookAndReturnsHttpsStatus204No_Content() throws Exception {
        BooksEntity booksEntity = TestDataUtil.createTestBooks(null);
        bookService.createUpdateBook(booksEntity.getIsbn(),booksEntity);

        mockMvc.perform(
                delete("/books/" + booksEntity.getIsbn())
